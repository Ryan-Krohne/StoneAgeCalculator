from tkinter import Tk, Button, Label, Entry, simpledialog
...
def update_totals():
    ...
    overallTotal = (tool_points + people_points + farm_points + hut_points+pointsFromHuts)
    minTotal = overallTotal + minCulture
    maxTotal = overallTotal + maxCulture
    overall_total_label.config(text=f"Min Score: {minTotal} Max Score: {maxTotal}")
...
def add_hut():
    ...
    try:
        hut_cost = simpledialog.askinteger("Hut Cost", "Enter the number of points the hut costs:")
...
tool_plus_button = Button(window, text="+", command=lambda: update_counter("tools", 1))
tool_plus_button.grid(row=0, column=2, padx=5, pady=5)

tool_minus_button = Button(window, text="-", command=lambda: update_counter("tools", -1))
tool_minus_button.grid(row=0, column=0, padx=5, pady=5)

# Create label for tool multiplier
toolMultiplier_label = Label(window, text=f"Multiplier: {toolMultiplier}", font=("Arial", 15))
toolMultiplier_label.grid(row=1, column=1, padx=5, pady=5)

# Create buttons for tool multiplier
toolMultiplier_plus_button = Button(window, text="+", command=lambda: update_counter("toolMultiplier", 1))
toolMultiplier_plus_button.grid(row=1, column=2, padx=5, pady=5)

toolMultiplier_minus_button = Button(window, text="-", command=lambda: update_counter("toolMultiplier", -1))
...
people_plus_button = Button(window, text="+", command=lambda: update_counter("people", 1))
people_plus_button.grid(row=3, column=2, padx=5, pady=5)

people_minus_button = Button(window, text="-", command=lambda: update_counter("people", -1))
people_minus_button.grid(row=3, column=0, padx=5, pady=5)

# Create label for peopleM counter
peopleM_label = Label(window, text=f"Multiplier: {peopleM}", font=("Arial", 15))
peopleM_label.grid(row=4, column=1, padx=5, pady=5)

# Create buttons for peopleM counter
peopleM_plus_button = Button(window, text="+", command=lambda: update_counter("peopleM", 1))
peopleM_plus_button.grid(row=4, column=2, padx=5, pady=5)

peopleM_minus_button = Button(window, text="-", command=lambda: update_counter("peopleM", -1))
...
farm_plus_button = Button(window, text="+", command=lambda: update_counter("farms", 1))
farm_plus_button.grid(row=6, column=2, padx=5, pady=5)

farm_minus_button = Button(window, text="-", command=lambda: update_counter("farms", -1))
farm_minus_button.grid(row=6, column=0, padx=5, pady=5)

# Create label for farm multiplier
farmMultiplier_label = Label(window, text=f"Multiplier: {farmMultiplier}", font=("Arial", 15))
farmMultiplier_label.grid(row=7, column=1, padx=5, pady=5)

# Create buttons for farm multiplier
farmMultiplier_plus_button = Button(window, text="+", command=lambda: update_counter("farmsM", 1))
farmMultiplier_plus_button.grid(row=7, column=2, padx=5, pady=5)

farmMultiplier_minus_button = Button(window, text="-", command=lambda: update_counter("farmsM", -1))
...
culture_plus_button = Button(window, text="+", command=lambda: update_counter("culture", 1))
culture_plus_button.grid(row=9, column=2, padx=5, pady=5)

culture_minus_button = Button(window, text="-", command=lambda: update_counter("culture", -1))
culture_minus_button.grid(row=9, column=0, padx=5, pady=5)

# Create label for culture total
culture_total_label = Label(window, text=f"Total Culture: 0", font=("Arial", 15))
...
hut_minus_button = Button(window, text="-", command=lambda: update_counter("huts", -1))
hut_minus_button.grid(row=11, column=0, padx=5, pady=5)

# Create label for hut multiplier
hutMultiplier_label = Label(window, text=f"Multiplier: {hutMultiplier}", font=("Arial", 15))
hutMultiplier_label.grid(row=12, column=1, padx=5, pady=5)

# Create buttons for hut multiplier
hutMultiplier_plus_button = Button(window, text="+", command=lambda: update_counter("hutMultiplier", 1))
hutMultiplier_plus_button.grid(row=12, column=2, padx=5, pady=5)

hutMultiplier_minus_button = Button(window, text="-", command=lambda: update_counter("hutMultiplier", -1))
...
overall_total_label = Label(window, text=f"Min Score: 0 Max Score: 0", font=("Arial", 15))
...
def update_counter(counter_name, value):
    ...
    if counter_name == "tools":
        ...
        toolMultiplier = max(toolMultiplier, 0)  # Ensure toolMultiplier counter is not negative
        ...
        farmMultiplier = max(farmMultiplier, 0)  # Ensure farmMultiplier counter is not negative
        ...
        hutMultiplier += value
        hutMultiplier = max(hutMultiplier, 0)  # Ensure hutMultiplier counter is not negative
